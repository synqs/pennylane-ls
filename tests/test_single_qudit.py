...
class TestSingleQuditDevice(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.testDevice = qml.device("synqs.sqs", shots = 50, username = self.username, password = self.password, blocking=True)
    ...
    def test_creation_with_user(self):
        ...
        '''
        testDevice = qml.device("synqs.sqs", shots = 50, username = self.username, password = self.password, blocking=True)
        self.assertEqual(testDevice.operations, {'load', 'rLx', 'rLz', 'rLz2'})
